...
def user_login():
    if request.method == 'POST':
        user = User.select().where(User.name == request.form['name']).get() # this is how you reach into the forms submitted data
...
def incomplete_task():
    ...
    return render_template('incomplete.jinja2', tasks=Task.select().where(Task.perfomed.is_null()))
